...
CREATE TABLE cmon.dockercontrol (id INT PRIMARY KEY AUTO_INCREMENT, hostname VARCHAR(255), host_ip VARCHAR(255), container_ip VARCHAR(255), platform VARCHAR(64), swarm_name VARCHAR(255), networks VARCHAR(255), active TINYINT NOT NULL DEFAULT 0);
...
CREATE TABLE cmon.container_job LIKE cmon.cmon_job;
FLUSH PRIVILEGES;
EOF
...
/deploy-container.sh 2>&1 | tee -a /var/www/html/proto/deploy.txt &
